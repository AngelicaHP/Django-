...
def checkout(request):
    ...
    # Obtener tipo de identificación predeterminado (puedes ajustar según tus datos)
        try:
            tipo_id = TipoIdentificacion.objects.first() # type: ignore
            canal = CanalCliente.objects.first() # type: ignore
            ...
            cliente = Cliente.objects.create(
                ...
                canal=canal,
                estado=EstadoEntidades.ACTIVO # type: ignore
             )
        except:
    ...
    if request.method == 'POST':
        # Crear la orden
        try:
            orden = OrdenCompraCliente.objects.create( # type: ignore
            ...
            for item in cart:
                ...
                ItemOrdenCompraCliente.objects.create( # type: ignore
                    item_id=uuid.uuid4(),
                    pedido=orden,
...
def order_detail(request, pedido_id):
    ...
    """
    orden = get_object_or_404(OrdenCompraCliente, pedido_id=pedido_id) # type: ignore
